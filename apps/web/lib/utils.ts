...
export function applyThemeToDom(element: HTMLElement, config: ThemeConfig) {
  ...
  element.style.setProperty("--filler-bg-overlay", String(config.bgOverlayOpacity ?? 0));
    ...
    element.style.setProperty("--filler-bg-image", "none");
    element.style.setProperty("--filler-bg-overlay", "0");
  }

  // Question specific styles
  if (config.questionBgColor) {
    element.style.setProperty("--filler-question-bg", config.questionBgColor);
  } else {
    element.style.removeProperty("--filler-question-bg");
  }
  if (config.questionBackdropBlur) {
    element.style.setProperty("--filler-question-blur", config.questionBackdropBlur);
  } else {
    element.style.removeProperty("--filler-question-blur");
  }
}
